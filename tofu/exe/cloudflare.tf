...
resource "cloudflare_record" "sandbox_wildcard" {
  zone_id = data.cloudflare_zone.this.id
...
resource "cloudflare_zero_trust_access_application" "coder" {
  ...
  # Hardening — reject browsers that don't speak modern TLS.
  http_only_cookie_attribute = true
  same_site_cookie_attribute = "lax"
}

resource "cloudflare_zero_trust_access_policy" "coder_owner" {
  application_id = cloudflare_zero_trust_access_application.coder.id
  zone_id        = data.cloudflare_zone.this.id
  name           = "owner only"
  precedence     = 1
  decision       = "allow"
