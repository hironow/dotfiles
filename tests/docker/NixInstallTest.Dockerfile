...
# Prepare for idempotency test: commit flake.lock generated by first run
RUN cd ~/dotfiles && \
    git add -A && \
    git -c user.name=test -c user.email=test@test.com commit -m "post-install state" --allow-empty
