...
| `google_sql_user.operator_iam` | IAM individual-user mapped to `var.owner_email`. **Read-only audit user** for Cloud SQL Studio (Console UI). Granted CONNECT on database + the Postgres 14+ predefined role `pg_read_all_data` (one role membership confers SELECT/USAGE on every relation regardless of owner — necessary because `cloudsqlsuperuser` cannot GRANT on tables owned by `coder_iam`). Write traffic stays exclusive to `coder.service` via `coder_iam`. |
