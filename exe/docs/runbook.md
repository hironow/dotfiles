...
## Cloud SQL Postgres password rotation

The password is generated by tofu's `random_password` and stored
as a Secret Manager version. To rotate:

```bash
# 1. Force tofu to regenerate.
just exe-replace random_password.coder_pg_password
just exe-apply         # writes new password to SQL user + Secret Manager

# 2. Force the VM to pick up the new /etc/default/coder.
...
The Coder server reconnects with the new password on the new VM's
first boot. No data loss; templates / users / workspaces survive.
